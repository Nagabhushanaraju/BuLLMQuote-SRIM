...
interface AccomplishAPI {
  ...
  // RFQ operations
  checkRfqExists(params: { rfqId: string }): Promise<{ exists: boolean }>;
  rfqGetFileVersions(params: { rfqId: string }): Promise<{
    versions: Array<{
      version: number;
      processedAt: string;
  ...
  rfqActivateVersion(params: { rfqId: string; bomId: string; volumeId: string }): Promise<{ success: boolean }>;
