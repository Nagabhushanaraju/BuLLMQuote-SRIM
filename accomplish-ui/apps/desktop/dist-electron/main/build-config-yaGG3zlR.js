...
function loadBuildConfig() {
	...
	const buildEnvPath = app.isPackaged ? path.join(process.resourcesPath, "build.env") : path.join(process.env.APP_ROOT || "", "build.env");
	let raw = {};
	let loadedFromFile = false;
	try {
		raw = parse(fs.readFileSync(buildEnvPath, "utf8"));
		loadedFromFile = true;
	} catch {}
	const parsed = buildConfigSchema.safeParse({
		...
		accomplishGatewayUrl: firstNonEmpty(raw.ACCOMPLISH_GATEWAY_URL, process.env.ACCOMPLISH_GATEWAY_URL),
		buildId: firstNonEmpty(raw.ACCOMPLISH_BUILD_ID, process.env.ACCOMPLISH_BUILD_ID),
		accomplishUpdaterUrl: firstNonEmpty(raw.ACCOMPLISH_UPDATER_URL, app.isPackaged ? void 0 : process.env.ACCOMPLISH_UPDATER_URL)
	});
	if (!parsed.success) {
		console.warn("[BuildConfig] Validation failed, using empty defaults:", parsed.error.message);
		cachedConfig = buildConfigSchema.parse({});
	} else cachedConfig = parsed.data;
	if (cachedConfig.buildEnvVersion) {
		const source = loadedFromFile && raw.BUILD_ENV_VERSION ? "build.env" : "process.env";
		console.log(`[BuildConfig] Loaded build config (buildEnvVersion=${cachedConfig.buildEnvVersion}, source=${source})`);
	} else if (cachedConfig.mixpanelToken || cachedConfig.gaApiSecret || cachedConfig.sentryDsn || cachedConfig.accomplishGatewayUrl || cachedConfig.accomplishUpdaterUrl) console.log("[BuildConfig] Loaded build config from process.env (dev / custom fallback)");
	else console.log("[BuildConfig] No build.env or env vars found — running in OSS mode");
	return cachedConfig;
}
/**
...
function getBuildConfig() {
	if (!cachedConfig) return loadBuildConfig();
	return cachedConfig;
}
...
//#endregion
export { isAutoUpdaterEnabled as a, isAnalyticsEnabled as i, getBuildConfig as n, isFreeMode as o, getBuildId as r, loadBuildConfig as s, getAppTier as t };

//# sourceMappingURL=build-config-yaGG3zlR.js.map