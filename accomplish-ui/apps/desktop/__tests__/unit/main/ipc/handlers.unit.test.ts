...
describe('IPC Handlers Integration', () => {
  ...
  describe('Multi-Provider API Key Validation', () => {
    ...
    it('api-key:validate-provider should handle failed response with 403 auth error', async () => {
      // Arrange — 401/403 are auth failures: key is invalid regardless of body
      vi.stubGlobal(
        'fetch',
        vi.fn().mockResolvedValue({
          ok: false,
      ...
      // Assert — validation logic returns a hardcoded message for 401/403
      expect(result.valid).toBe(false);
      expect(result.error).toBe('Invalid API key');
    });

    it('api-key:validate-provider should treat non-auth errors as valid', async () => {
      // Arrange — 500 errors are NOT auth failures; they mean the key authenticated
      // but the probe request was rejected (e.g. retired model, rate-limit). The
      // validator treats these as valid to avoid falsely rejecting working keys.
      vi.stubGlobal(
        'fetch',
        vi.fn().mockResolvedValue({
          ok: false,
