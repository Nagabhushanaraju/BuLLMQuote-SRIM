...
describe('API Key Validation', () => {
  ...
  describe('validateApiKey', () => {
    ...
    describe('Error handling', () => {
      ...
      it('should treat non-auth error responses as valid', async () => {
        // Non-401/403 errors mean the key authenticated but the probe request
        // was rejected (e.g. retired model, rate-limit). Treat as valid to avoid
        // falsely rejecting working keys.
        vi.mocked(fetch).mockResolvedValueOnce({
          ok: false,
          status: 500,
      ...
      it('should treat error response without message as valid', async () => {
        vi.mocked(fetch).mockResolvedValueOnce({
          ok: false,
          status: 500,
          json: async () => ({}),
      ...
      it('should treat JSON parse errors as valid (non-auth failure)', async () => {
        vi.mocked(fetch).mockResolvedValueOnce({
          ok: false,
          status: 500,
          json: async () => {
