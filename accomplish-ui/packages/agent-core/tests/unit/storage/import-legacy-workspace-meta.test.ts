...
describe('importLegacyWorkspaceMeta', () => {
  ...
  // Skip on Windows: fs.chmodSync is a no-op there so the read-only simulation
  // never triggers EACCES, making the test's premise impossible to simulate.
  it.skipIf(process.platform === 'win32')(
    '18i. unlinkLegacyTriplet failure (simulated via read-only file) leaves status missing',
    () => {
      if (!Database || !importModule) return;
      const db = openMainDb();
